...
function initTSN(config) {
	var tag;

	// Entity, comments, CDATA, closed taq.
	var tagStart = '(?:&' + config
	...
	function TSN(path, isInline) {
		...
		fullPath = config.templatePath + '/' + path;
		...
		if (isInline !== true) {
			try {
				path = LIB.fileSystem.readFileSync(fullPath, config.encoding);
			} catch (error) {
				return error;
			}
		...
		cRegExp = new RegExp((isXML === false && config.type == 'html') ? regExp.tagHTML : regExp.tagXML, 'gi');
