...
Parser.prototype.onOpen = function (node) {
	if (nodeAPI.hasOwnProperty(node.name)) {
		...
		var entityRegExp = new RegExp('&' + this.config.namespace + '.([a-z0-9\\-_\\.]+)?;');
				...
				if (node.attributes.hasOwnProperty(name)) {
					switch (attrDefinition.type) {
						case 'string':
							...
							if (entityRegExp.text(attrValue)) {
						...
						case 'number':
							if (isNaN(attrValue)) {
								this._error('The value of the attribute "' + name + '" must be a number', node);
							}
							break;
						default:
					}
					...
					if (attrDefinition.required === true) {
						this._error('Attribute "' + name + '" is not defined.', node);
						node.hasError = true;
						return;
