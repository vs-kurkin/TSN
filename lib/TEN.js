...
TEN.compile = function (source, config) {
	...
	var template = new Template(source, config);
...
TEN.render = function (template, context, stream) {
	...
	return template.call(context, stream);
};
...
TEN.inCache = function (template) {
	switch (typeof template) {
		case 'string':
			...
			break;
		case 'function':
			for (var path in TEN.cache) {
				if (TEN.cache.hasOwnProperty(path) && TEN.cache[path] === template) {
					return true;
				}
			}
			return false;
...
var Template = require(LIB.path.join(__dirname, 'Template.js'));
