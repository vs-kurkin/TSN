...
function __error (error, template, stack, name, line, column) {
	...
	if (stack) {
		stack.queue.push('<h3>RenderError:<br /> ' + error.toString() + '<br />Template: ' + template.path + '<br />Node name: ' + name + '<br />Line: ' + line + '<br />Column: ' + column + '</h3>');
	}
}
...
EventHandler.prototype.onEndElementNS = function () {
	...
	current.value = current.toString(this);
...
Node.prototype.toString = function (eventHandler) {
	var
	...
	for (name in attributes) {
		if (attributes.hasOwnProperty(name)) {
			...
			switch (typeof value) {
				...
				case 'object':
					result += ' ' + name + '=\\"" + (__error.data = ["' + value.name + '", ' + value.line + ', ' + value.column + '], ' + value.value + ') + "\\"';
					break;
			}
		}
	}
	...
	if (this.mayBeError === true) {
		result = 'try {' + result + '} catch (error) {' +
				'__error(error, __template, ' + (eventHandler.config.debug === true ? '__stack' : 'null') + ', __error.data[0], __error.data[1], __error.data[2]);' +
				'delete __error.data;' +
			'}';
