 ...
 */
module.exports = function (ten, source) {
	var
        options = ten.options,
		args = [ten, Buffer, __error],
	...
	argumentsName.push(
		'var ' +
			...
			'__TEN = this,' +
			'__path = "' +
			(typeof options.path === 'string' ? options.path.replace(/("|(?:\r\n)|\r|\n|\\)/g, '\\$1') : 'undefined') +
			'";' +
...
function __error (error, template, buffer, name, line, column) {
	...
	if (buffer) {
		buffer.push('<h3>RenderError:<br /> ' +
			error.toString() + '<br />Template: ' + template.path + '<br />Node name: ' + name + '<br />Line: ' + line + '<br />Column: ' + column + '</h3>');
	}
}
