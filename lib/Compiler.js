	...
	if (current.namespace === NS) {
		if (nodeAPI.hasOwnProperty(current.name)) {
			...
			if (result instanceof Error) {
				...
				result = typeof current.parse === 'function' ? current.parse(document, config, this.API, TEN) : true;

				if (result instanceof Error) {
					error = result.message;
				} else {
					code = current.compile(config);

					if (current.initCode === true) {
						...
						if (!current.parent.firstState) {
							current.parent.firstState = newState;
						}

						current.parent.state = newState;
						current.parent.code += code;
					}
	...
	function replace (result, type, name) {
		switch (type) {
			case '!':
				switch (name) {
					case 'code':
						if (node.state === STATE_TEXT && node.templateType === TEMPLATE_TYPE_CODE) {
							node.code += '");';
							node.state = STATE_CODE;
						}
