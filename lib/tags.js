...
(function (API) {
	...
	API.block = {
		...
		parse: function (document, config, TEN) {
			var attributes = this.attributes;
			...
			if (name === '') {
				...
				switch (attributes.type) {
					case 'default':
						...
						break;
					case 'local':
						this.template = '_block[' + name + '] = ' + this.template;
						break;
					case 'global':
						this.template = '__TEN.block[' + name + '] = ' + this.template;
						break;
					default:
	...
	API.render = {
		...
		start: function () {
			return 'var _data,' +
				...
				'if (__TEN.hasOwnProperty("parent")) {' +
					'_data = __TEN.parent._data;' +
					'_blankBlock = __TEN.parent._blankBlock;' +
				'} else {' +
					'_data = {};' +
					'_blankBlock = function () {};' +
				'}';
		},
		parse: function (parser) {
			...
			if (attributes.hasOwnProperty('file')) {
				...
				this.template = '(' +
					...
					'_block[' + blockName + '] || ' +
					'__TEN.block[' + blockName + '] || ' +
					'_blankBlock)' +
						'.call(/*!context*/, __stack, _localBlock);';
			} else {
