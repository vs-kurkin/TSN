...
this.root = {
	...
	template: '/*!code*/',
	mayBeError: false,
	templateType: CONST.TEMPLATE_TYPE_INHERIT,
	print: false
};
...
this.context = {
	...
	parse: function () {
		if (!this.attributes.hasOwnProperty('object')) {
			...
			this.templateType = CONST.TEMPLATE_TYPE_INHERIT;
	...
	template:
		'(function () {' +
			'/*!code*/' +
		'}).call(/*@object*/);',
	templateType: CONST.TEMPLATE_TYPE_CODE,
	mayBeError: true,
	print: false
};
	...
	parse: function () {
		...
		if (attributes.hasOwnProperty('value')) {
			...
			this.template =
				'(function (__buffer) {' +
					'/*!code*/' +
					'__buffer.end(function (result) {' +
						'_data[/*@key*/] = ' + this.template +
					'});' +
				'}).call(/*!context*/, new __Buffer());';
		}
	},
	templateType: CONST.TEMPLATE_TYPE_CODE,
	mayBeError: true,
...
(function (API) {
	...
	API.block = {
		...
		template:
			'function (__buffer, __localBlock) {' +
				'var _context = this;' +
				'/*!code*/' +
			'};',
		templateType: CONST.TEMPLATE_TYPE_CODE,
	...
	API.render = {
		...
		parse: function (document, options, API, TEN) {
			var attributes = this.attributes;

			if (attributes.hasOwnProperty('file')) {
				if (attributes.file.charAt(0) !== '/') {
					if (!options.hasOwnProperty('dir')) {
						options.dir = options.pathRoot;
					}

					attributes.file = LIB.path
						.relative(options.pathRoot, LIB.path.resolve(options.dir, attributes.file))
						.replace(/\\/g, '\\\\');
				}
				...
				if (!attributes.hasOwnProperty('options')) {
					...
					delete options.path;
					attributes.options = options.inheritConfig === true ? JSON.stringify(options) : 'null';
					options.path = path;
				}
				...
				this.template =
					...
					'__TEN' +
						'.compileFromFile(/*@file*/, /*@options*/)' +
						'.render(/*!context*/, __stream, __buffer);' +
...
//TODO: выпиливать JS-комментарии.
this.script = {
	attributes: {
		type: {
			defaultValue: 'global'
	...
	parse: function () {
		...
		this.template =
			'(function (__buffer) {' +
				...
				'__buffer.end();' +
				'});' +
			'})(new __Buffer(__buffer));';
	},
	templateError: '__buffer.wait--;',
	templateType: CONST.TEMPLATE_TYPE_CODE,
	mayBeError: true,
	print: false
};
...
this.attribute = (function () {
	function getNode (node) {
		...
		while (parent) {
			if (parent.namespace !== CONST.NS || parent.name === 'element') {
				return parent;
			}
	...
	return {
		...
		parse: function (document, options, API, TEN) {
			...
			if (attributes.hasOwnProperty('value')) {
				this.template = this.attributes.value;
				this.print = this.mayBeError = true;

				code = '");' + this.compile(options) + '__buffer.push("';
				this.print = this.mayBeError = false;
				this.templateType = CONST.TEMPLATE_TYPE_INHERIT;
			} else {
				this.template = '/*!code*/';

				code = this.compile(options);

				/*if (this.state === CONST.STATE_CODE) {
					code += '__buffer.push("';
					this.state = node.state;
				}

				if (node.state === CONST.STATE_CODE) {
					code = '");' + code;
				}*/
			}

			node.attributes[attributes.name] = code;
