			...
			if (attributes.hasOwnProperty('file')) {
				if (attributes.file.charAt(0) !== '/') {
					if (!config.hasOwnProperty('dir')) {
						config.dir = config.pathRoot;
					}

					attributes.file = LIB.path
						.relative(config.pathRoot, LIB.path.resolve(config.dir, attributes.file))
						.replace(/\\/g, '\\\\');
				}
...
this.attribute = (function () {
	...
	return {
		...
		parse: function (document, config, API, TEN) {
			var
			...
			if (attributes.hasOwnProperty('value')) {
				...
				code = '");' + this.compile(config) + '__buffer.push("';
				this.print = this.mayBeError = false;
				this.templateType = CONST.TEMPLATE_TYPE_INHERIT;
			} else {
				this.template = '/*!code*/';

				code = this.compile(config);
