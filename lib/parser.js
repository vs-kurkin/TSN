...
(function () {
    ...
    XMLP._errs = [];
    XMLP._errs[0] = "PI: missing closing sequence";
    XMLP._errs[1] = "DTD: missing closing sequence";
    XMLP._errs[2] = "Comment: missing closing sequence";
    XMLP._errs[3] = "CDATA: missing closing sequence";
    XMLP._errs[4] = "Element: missing closing sequence";
    XMLP._errs[5] = "Entity: missing closing sequence";
    XMLP._errs[6] = "PI: target is required";
    XMLP._errs[7] = "Element: cannot be both empty and closing";
    XMLP._errs[8] = "Element: name must immediatly follow \"<\"";
    XMLP._errs[9] = "Element: \"<\" not allowed in element names";
    XMLP._errs[10] = "Attribute: values are required and must be in quotes";
    XMLP._errs[11] = "Element: \"<\" not allowed in attribute names";
    XMLP._errs[12] = "Attribute: \"<\" not allowed in attribute values";
    XMLP._errs[13] = "Attribute: duplicate attributes not allowed";
    XMLP._errs[14] = "Entity: unknown entity";
    XMLP._errs[15] = "Infininte loop";
    XMLP._errs[16] = "Document: only comments, processing instructions, or whitespace allowed outside of document element";
    XMLP._errs[17] = "Element: must be nested correctly";
    ...
    XMLP.prototype._checkStructure = function (iEvent) {
        ...
        if (XMLP._STATE_MISC == this.m_iState) {
            if ((XMLP._ELM_B == iEvent) || (XMLP._ELM_E == iEvent) || (XMLP._ELM_EMP == iEvent) || (XMLP._EVT_DTD == iEvent)) {
                return this._setErr(XMLP.ERR_DOC_STRUCTURE);
            }
    ...
    XMLP.prototype._findAttributeIndex = function (name) {
        ...
        for (i = 0; i < this.m_atts.length; i++) {
    ...
    XMLP.prototype._parse = function () {
        ...
        if (this.m_iP == mXML.length) {
            return XMLP._NONE;
        }

        var fc = mXML.charAt(this.m_iP);

        if (fc !== '<' && fc !== '&') {
            return this._parseText(this.m_iP);
        } else {
            switch (this.m_iP) {
                case mXML.indexOf("<?", this.m_iP):
                    return this._parsePI(this.m_iP + 2);
                case mXML.indexOf("<!DOCTYPE", this.m_iP):
                    return this._parseDTD(this.m_iP + 9);
                case mXML.indexOf("<!--", this.m_iP):
                    return this._parseComment(this.m_iP + 4);
                case mXML.indexOf("<![CDATA[", this.m_iP):
    ...
    XMLP.prototype._parsePrefixAndElementName = function (elementlabel) {
        ...
        return {
            prefix: length === 1 ? '' : splits[0],
            name: length === 1 ? elementlabel : splits[1]
        };
    };
    ...
    XMLP.prototype._parseAttribute = function (iB, iE) {
        var iNB, iNE, iEq, iVB, iVE, iRet, cQuote, strN, strV;
    ...
    XMLP.prototype._parseElement = function (iB) {
        var iE, iDE, iNE, iRet, iType, strN, iLast;
    ...
    XMLP.prototype._replaceEntities = function (strD, iB, iE) {
        ...
        this._setContent(XMLP._CONT_ALT, strRet, null);
    ...
    XMLP.prototype._replaceEntity = function (strD, iB, iE) {
        ...
        this._setContent(XMLP._CONT_ALT, strEnt, null);

        return XMLP._ENTITY;
    };

    XMLP.prototype._setContent = function (iSrc, iB, iE) {
    ...
    //with XML for <SCRIPT>
    var SaxParser = function (eventHandler) {
    ...
    SaxParser.prototype.parseString = function (strD) {
        ...
        if (!this.mParser) {
            this.mParser = new XMLP(strD);
            startNew = false;
        } else {
            this.mParser.continueParsing(strD);
            startNew = true;
        }
    ...
    SaxParser.prototype.getValueByName = function (name) {
        return this.mParser.getAttributeValueByName(name);
    };
    ...
    SaxParser.prototype._fireError = function (strMsg) {
        ...
        if (this.m_hndDoc && this.m_hndDoc.onError) {
            this.m_hndDoc.onError(this.m_strErrMsg);
        }
    };
    ...
    SaxParser.prototype._fireEvent = function (iEvt) {
        ...
        switch (iEvt) {
            case SaxParser.DOC_B:
                listener = "onStartDocument";
                arguments.splice(0);
                break;

            case SaxParser.DOC_E:
                listener = "onEndDocument";
                arguments.splice(0);
                break;

            case SaxParser.ELM_B:
                listener = "onStartElementNS";
                arguments.splice(3);
                break;

            case SaxParser.ELM_E:
                listener = "onEndElementNS";
                arguments.splice(2);
                break;

            case SaxParser.CHARS:
                listener = "onCharacters";
                arguments.splice(1);
                break;

            case SaxParser.PI:
                listener = "processingInstruction";
                arguments.splice(1);
                break;
        ...
        if (listener && typeof this.m_hndDoc[listener] === "function") {
            arguments.unshift(this);
            this.m_hndDoc[listener].apply(this.m_hndDoc, arguments);
        }
    };
        ...
        while (!this.m_bErr) {
            switch (parser.next()) {
                case XMLP._ELM_B:
                    ...
                    this._fireEvent(SaxParser.ELM_B, nameObject.name, this.mParser.m_atts, (nameObject.prefix === '') ? null : nameObject.prefix);
                    break;

                case XMLP._ELM_E:
                    nameObject = parser._parsePrefixAndElementName(parser.getName());
                    this._fireEvent(SaxParser.ELM_E, nameObject.name, (nameObject.prefix === '') ? null : nameObject.prefix);
                    break;
